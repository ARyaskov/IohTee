...
//   const fakeChan = new PaymentChannel(
//     '0xcafe',
//     '0xbeef',
//     '123',
//     new BigNumber(10),
...
//     uuidStub = sinon
//       .stub(uuid, 'v4')
//       .returns('0e29e61f-256b-40b2-a628-0f8181a1b5ff')
//
...
//       machOpts,
//     )
//     channelEthContract.getSettlementPeriod = sinon
//       .stub()
//       .resolves(ChannelManager.DEFAULT_SETTLEMENT_PERIOD)
//     channelTokenContract.getSettlementPeriod = sinon
//       .stub()
//       .resolves(ChannelManager.DEFAULT_SETTLEMENT_PERIOD)
//   })
//
...
//         .openChannel('0xcafe', '0xbeef', new BigNumber(10))
//         .then(() => {
//           expect(
//             (channelContract.open as sinon.SinonStub).calledWith(
//               '0xcafe',
//               '0xbeef',
//               new BigNumber(100),
//               ChannelManager.DEFAULT_SETTLEMENT_PERIOD + 1,
//             ),
//           ).toBe(true)
//         })
//     })
...
//       return channelManager
//         .openChannel('0xcafe', '0xbeef', new BigNumber(1))
//         .then(() => {
//           const fakeChan2 = {
//             ...fakeChan,
...
//           }
//           expect(
//             (channelsDao.save as sinon.SinonStub).calledWith(fakeChan2),
//           ).toBe(true)
//         })
//     })
//
...
//       channelContract.channelById = sinon
//         .stub()
//         .withArgs(id)
//         .resolves([
//           fakeChan.sender,
...
//       channelsDao.updateSettlingUntil = sinon
//         .stub()
//         .withArgs(id, 100)
//         .resolves()
...
//         expect(
//           (channelsDao.updateState as sinon.SinonStub).calledWith(id, 1),
//         ).toBe(true)
//       })
//     })
//
...
//         expect(
//           (channelsDao.updateState as sinon.SinonStub).calledWith(id, 2),
//         ).toBe(true)
//       })
//     })
//
...
//           0,
//         ])
//       paymentsDao.firstMaximum = sinon
//         .stub()
//         .withArgs(id)
//         .resolves(
//           new Payment({
...
//           }),
//         )
//       channelContract.claim = sinon
//         .stub()
//         .withArgs(channel.receiver, channel, channel.value, 1, '0x01', '0x02')
...
//         savedChannel = new PaymentChannel(
//           '0xcafe',
//           '0xbeef',
//           id,
//           new BigNumber(1),
...
//         expect((channelsDao.save as sinon.SinonStub).lastCall.args[0]).toEqual(
//           savedChannel,
//         )
//       })
//
...
//       channelsDao.firstById = sinon
//         .stub()
//         .withArgs(id)
//         .resolves({} as PaymentChannel)
...
//         channelManager.deposit(id, new BigNumber(1)),
//       ).then(() => {
...
//         .withArgs(id)
//         .resolves({} as PaymentChannel)
//       channelsDao.deposit = sinon
//         .stub()
//         .withArgs('0xcafe', id, new BigNumber(10))
//         .resolves()
//       channelContract.channelById = sinon
//         .stub()
//         .withArgs(id)
...
//           0,
//         ])
//       channelContract.deposit = sinon
//         .stub()
//         .resolves({ tx: '123abc' } as TransactionResult)
//
//       return channelManager
//         .deposit(id, value)
...
//       channelContract.channelById = sinon.stub().resolves(Promise.resolve([]))
//       return expectsRejection(
//         channelManager.nextPayment(id, new BigNumber(6), ''),
//       )
//     })
//
...
//         ])
//       return expectsRejection(
//         channelManager.nextPayment(id, new BigNumber(9), ''),
//       )
//     })
//
...
//           0,
//         ])
//       paymentManager.buildPaymentForChannel = sinon
//         .stub()
//         .withArgs(channel, sinon.match.object, sinon.match.object, '')
//         .callsFake(
//           (
...
//
//       return channelManager
//         .nextPayment(id, new BigNumber(8), '')
...
//
//       expect(
//         (channelsDao.saveOrUpdate as sinon.SinonStub).calledWith({
//           sender: 'send',
//           receiver: 'recv',
//           channelId: '0xdead',
//           value: new BigNumber(10),
...
//       tokensDao.save = sinon
//         .stub()
//         .withArgs('token', payment.channelId)
//         .resolves()
...
//       channelsDao.firstById = sinon
//         .stub()
//         .withArgs(newChan.channelId)
//         .resolves(newChan)
//
//       return expectsRejection(channelManager.acceptPayment(payment)).then(
//         () => ({ signature, newChan }),
//       )
//     }
//
//     it('should close the channel if the payment is invalid and a channel exists', () => {
//       return testNextPayment().then(
//         (res: { signature: Signature; newChan: any }) =>
//           expect(
//             (channelContract.claim as sinon.SinonStub).calledWith(
//               fakeChan.receiver,
//               res.newChan.channelId,
//               new BigNumber(0.5),
//               res.signature,
//             ),
//           ).toBe(true),
...
//
//       return testNextPayment().then(() =>
//         expect((channelContract.claim as sinon.SinonStub).notCalled).toBe(true),
//       )
//     })
//   })
//
...
//
//       return channelManager
//         .requireOpenChannel('0xcafe', '0xbeef', new BigNumber(1))
